...
- **One default destination.** "Who gets paged" is configuration, not
  code, and it is one decision rather than a tree of them.
- **Grouping.** Related alerts batch into one notification instead of
  paging once per instance, on fixed rules rather than per-rule tuning.
...
agent carries. That last one is the constraint the design did not meet,
and it is recorded as a design signal rather than hidden. The first two
shape most of the design, and they make the surface better for humans
too.
...
Two more rules come from the storage design and are stated there: what a
lost row is allowed to mean, and what may never be written into an
append-only store.

## Where to look next

Main already proves the evaluate-and-send loop, so the risk is not the
engine. What shipped and what is still open are in
