...
**Details:** step 4 of Order of work in `../02-alerting-clickhouse-surface.md`; blockers 3 and 4 in the
...
- [ ] Both window bounds are tested invariants: wider than outage plus retry span plus the longest journal-writing transaction (a slow registry apply), narrower than `min(tenant logs_days, the 90-day journal retention in server/alerting/maintenance/cleanup.ts)`
