...
- [**rate-limiting-ingest-keys**](todo/ideas/rate-limiting-ingest-keys.md) — Enforce per-key rate limits on the public OTLP ingest path so a single noisy or runaway client can't dominate ingest capacity. Configurable per-key in the dashboard.
