...
describe("/alerts/slos route", () => {
  ...
  it("resumes a paused SLO", async () => {
    mocks.listAlertingSlos.mockResolvedValue([alertingSlo({ paused: true })]);
    const user = userEvent.setup();
    renderSlosRoute();
    ...
    await waitFor(() =>
      expect(mocks.resumeAlertingSlo).toHaveBeenCalledWith({
        data: { sloId: SLO_ID },
      }),
    );
  });
  ...
  it("passes the active preview name into the SLO listing query", async () => {
    ...
    expect(mocks.listAlertingSlos).toHaveBeenCalledWith({
      data: { preview: "feat/slo-preview" },
    });
  });

  it("fails to the shared error card when the listing errors", async () => {
    mocks.listAlertingSlos.mockRejectedValue(new Error("alerting unavailable"));
  ...
  it("orders by name, independent of status: a firing SLO does not jump the list", async () => {
    mocks.listAlertingSlos.mockResolvedValue([
      alertingSlo({ id: "z", name: "z-svc" }),
      alertingSlo({ id: "a", name: "a-svc" }),
    ]);
    ...
    mocks.getAlertingSloStatus.mockImplementation(({ data: { sloId } }) =>
  ...
  it("paginates when there are more SLOs than fit on a page", async () => {
    const many = Array.from({ length: 12 }, (_, i) => {
      ...
      return alertingSlo({ id: `slo-${n}`, name: `svc-${n}` });
    });
    mocks.listAlertingSlos.mockResolvedValue(many);
    const user = userEvent.setup();
    renderSlosRoute();
