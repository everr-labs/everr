...
type SilenceGroup = "active" | "scheduled" | "expired" | "cancelled";

function groupOf(s: AlertingSilence, now: number): SilenceGroup {
  // Cancelling collapses the window, so a cancelled silence that never started
  // still reads as scheduled by the dates alone. Check the stamp first.
...
const GROUPS: {
  ...
  { key: "cancelled", title: "Cancelled", cancellable: false },
];
...
export function SilencesPanel({ onNewSilence }: { onNewSilence: () => void }) {
  ...
  const cancel = useMutation({
    mutationFn: (id: string) => expireAlertingSilence({ data: { id } }),
    onSuccess: () => {
  ...
  const grouped = GROUPS.map((g) => ({
    ...
    rows: silences
      ...
      .sort((a, b) =>
        // Still open: soonest-ending first. Already over: newest first.
        g.cancellable
