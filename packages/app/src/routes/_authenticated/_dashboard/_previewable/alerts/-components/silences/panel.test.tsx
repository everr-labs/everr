...
describe("SilencesPanel", () => {
  ...
  it("cancels a silence that is still ahead of its end, but not an expired one", async () => {
    ...
    expect(mocks.expireAlertingSilence).not.toHaveBeenCalled();
    expect(
  ...
  // Cancelling collapses the window rather than deleting the row, so a
  // cancelled silence that never started still has future dates on it.
  it("groups a cancelled silence as cancelled even when its window is ahead", async () => {
    mocks.listAlertingSilences.mockResolvedValue([
      alertingSilence({
        id: "sil-cancelled",
        comment: "called off",
    ...
    await screen.findByText("called off");
    expect(
      screen.getByRole("heading", { name: "Cancelled" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: "Scheduled" }),
    ).not.toBeInTheDocument();
    expect(
