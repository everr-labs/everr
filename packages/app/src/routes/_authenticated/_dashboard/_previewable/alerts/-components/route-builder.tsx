...
import { AlertingDisclosureTrigger, alertingErrorMessage } from "./shared";
...
function routeInput(route: AlertingRoute, priority: number) {
  return {
...
export function RouteBuilder({
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit) {
        return updateAlertingRoute({ data: { id: route.id, input } });
      }
      ...
      return createAlertingRoute({ data: input });
    },
    onSuccess: () => {
      onCancel();
    ...
    >
      ...
      >
        ...
        <Collapsible open={timingOpen} onOpenChange={setTimingOpen}>
          <AlertingDisclosureTrigger open={timingOpen}>
            ...
            <div className="space-y-3 rounded-b-md border-x border-b border-border/60 p-3">
                ...
                <OptionCombobox
                  ...
                  onChange={(value) => {
                    ...
                    if (nextMode === "labels" && groupBy.length === 0) {
                      setGroupBy([...ALERTING_DEFAULT_GROUP_BY]);
                    }
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <DurationField
                  id="route-group-wait"
                  label="Group wait (s)"
                  placeholder={String(ALERTING_DEFAULT_GROUP_WAIT_SECS)}
                  value={groupWait}
                ...
                <DurationField
                  ...
                  placeholder={String(ALERTING_DEFAULT_GROUP_INTERVAL_SECS)}
                  value={groupInterval}
