...
export function AlertEventFeed({
  ...
  /** Canonical qualified slugs to include. */
  ...
  const columns: Column<AlertEventLogRow>[] = [
    ...
    },
    {
      header: "Event",
      cell: (e) => {
        const status = alertingEventStatus(e.eventType);
        return (
          <span className="inline-flex items-center gap-1.5">
            {status ? (
              <AlertingAlertStatusLabel status={status} />
            ) : (
      ...
      <CardContent>
        {history.isError && (
          <div className="px-3 pb-2 text-xs text-destructive">
            Event history unavailable ({alertingErrorMessage(history.error)}).
          </div>
        )}
        {history.isPending ? (
