...
import { type CcPipelineFacts, CcPipelineStrip } from "./pipeline-strip";

const FACTS: CcPipelineFacts = {
  watchingRules: 4,
  pausedRules: 1,
  watchingSlos: 2,
  firing: 3,
...
describe("CcPipelineStrip", () => {
  it("reads out all four stages", async () => {
    renderStrip(<CcPipelineStrip facts={FACTS} />);
  ...
  it("flags unrouted firing instances on the Notifying stage", async () => {
    renderStrip(<CcPipelineStrip facts={{ ...FACTS, unroutedFiring: 3 }} />);
  ...
  it("says all quiet when nothing is firing or pending", async () => {
    renderStrip(
      <CcPipelineStrip facts={{ ...FACTS, firing: 0, pending: 0 }} />,
    );
