 ...
 * never touched.
 *
 * Alert rules and SLOs also reference the preview row with ON DELETE CASCADE,
 * so their definitions and dependent evaluation state are removed by the same
 * database transaction. Already-queued evaluation jobs safely no-op when
 * their definition no longer exists.
 */
