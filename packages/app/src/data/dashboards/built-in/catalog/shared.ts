...
export const needsTraces: BuiltinRequirement = {
  kind: "signal",
  match: "traces",
  label: "no traces",
};

export const needsLogs: BuiltinRequirement = {
  kind: "signal",
  match: "logs",
  label: "no logs",
};

export const needsMetrics: BuiltinRequirement = {
  kind: "signal",
  match: "metrics",
  label: "no metrics",
};

export const needsSpanAttribute = (match: string): BuiltinRequirement => ({
  kind: "span-attribute",
  match,
  label: `no ${match}*`,
});

export const needsMetricNamespace = (match: string): BuiltinRequirement => ({
  kind: "metric",
  match,
  label: `no ${match}*`,
});

export const needsLogAttribute = (match: string): BuiltinRequirement => ({
  kind: "log-attribute",
  match,
  label: `no ${match}*`,
});
...
/** The error-rate tile five built-ins share, optionally scoped further. */
...
/** The P95 tile four built-ins share, in milliseconds. */
