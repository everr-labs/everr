import type { CcSlo } from "@/data/cc/types";
import { fromCcSlo, isOwnedSlo } from "./mapping";

export type SloPreviewScope = {
  id: string;
  repoid: string;
};

/**
 * A no-op now: identity rides on CC's first-class `name`, which is already
 * the authored project/slug for both a live SLO and its preview copy (the
 * `namespace` tells them apart, not the name), so there is never a mismatch
 * to fix up. Kept so existing call sites don't need to change.
 */
export function withAuthoredSloName<T extends CcSlo>(slo: T): T {
  return slo;
}

/**
 * CC stores preview SLOs as real, suppressed SLOs sharing their live
 * counterpart's `name`, distinguished only by `namespace`. For read-side
 * surfaces, show the active preview as an overlay over live state: preview
 * rows replace their live counterpart, live rows from covered repos
 * disappear when removed in preview, and preview rows for other preview ids
 * stay hidden.
 */
export function visibleSlosForPreview<T extends CcSlo>(
  slos: readonly T[],
  scopes: readonly SloPreviewScope[] | null,
): T[] {
  if (scopes === null) {
    return slos.filter((slo) => fromCcSlo(slo).previewId === null);
  }

  const scopeRepoidByPreviewId = new Map(
    scopes.map((scope) => [scope.id, scope.repoid]),
  );
  const coveredRepoids = new Set(scopes.map((scope) => scope.repoid));

  const previewRows = slos.filter((slo) => {
    const view = fromCcSlo(slo);
    return (
      view.previewId !== null &&
      isOwnedSlo(slo) &&
      scopeRepoidByPreviewId.get(view.previewId) === view.repoid
    );
  });

  const liveRows = slos.filter((slo) => {
    const view = fromCcSlo(slo);
    if (view.previewId !== null) return false;
    if (!isOwnedSlo(slo)) return true;
    if (!coveredRepoids.has(view.repoid)) return true;
    return false;
  });

  return [...previewRows, ...liveRows];
}
