...
function addCounts(
  ...
  return {
    ...
    sloEvaluations: total.sloEvaluations + batch.sloEvaluations,
...
export async function cleanupAlertingHistory(options?: {
  ...
  for (let batch = 0; batch < maxBatches; batch += 1) {
    const counts = await db.transaction(async (tx) => {
      ...
      `);
      const sloEvaluations = await tx.execute(sql`
        WITH doomed AS (
          SELECT ctid
        ...
        DELETE FROM slo_evaluations AS target
        USING doomed
        WHERE target.ctid = doomed.ctid
      `);
      ...
      return {
        ...
        sloEvaluations: deletedRows(sloEvaluations),
