...
import { cleanupAlertingHistory } from "./maintenance";
...
describe("cleanupAlertingHistory", () => {
  ...
  it("protects unfinished events, active groups, and delivery history", async () => {
    ...
    const eventDelete = sqlText(mocks.execute.mock.calls[2][0]);
    ...
    const deliveryDelete = sqlText(mocks.execute.mock.calls[3][0]);
    ...
    const groupDelete = sqlText(mocks.execute.mock.calls[4][0]);
