...
import { ALERTING_DEFAULT_GROUP_INTERVAL_SECS } from "@/data/alerting/delivery/defaults";
...
import { type AlertingHarness, useAlertingHarness } from "./testing/harness";
...
function onlyEvaluationRow(active: AlertingHarness): EvaluationHistoryRow {
  const rows = active.clickhouse
    .historyRows()
...
describe("the alerting pipeline's capacity bounds", () => {
  it("claims exactly the cap from a 501-member group, taking every never-yet-flushed newcomer over the one member already flushed once", async () => {
    ...
    const [staleEvent] = await harness()
      .db.select()
      .from(alertEvents)
      .where(eq(alertEvents.eventType, "instance_fired"));
    const [staleMemberAfterFirstFlush] = await harness()
      .db.select()
    ...
    const [{ version }] = await harness()
      .db.select({ version: alertDefinitions.version })
    ...
    const membersBeforeCappedFlush = await harness()
      .db.select()
      .from(alertNotificationGroupEvents);
    ...
    const afterCappedFlush = await harness()
      .db.select()
      .from(alertNotificationGroupEvents);
  ...
  it("the scanner's batch of 5000 claims a 5001-rule backlog over two scans", async () => {
    ...
    const stillUnenqueued = await harness()
      .db.select()
      .from(alertDefinitions)
  ...
  it("skips a rule whose last_enqueued_at is newer than next_evaluation_at, and re-enqueues it once that stamp passes the stale cutoff", async () => {
    ...
    const [stillSkipped] = await harness()
      .db.select()
      .from(alertDefinitions)
      .where(eq(alertDefinitions.id, rule.id));
