...
export function StatusHistoryVisualization({
  ...
  if (!model || span <= 0 || !model.lanes.some((l) => l.cells.length > 0)) {
    const message = !data
              ...
              >
                ...
                <div className="relative min-w-0 flex-1 overflow-hidden">
                  {lane.cells.map((cell) => (
                    ...
                    <div
                      key={cell.ts}
                      className="absolute flex items-center justify-center overflow-hidden rounded-[2px] px-1"
                      style={{
                        left: `${toPct(cell.start)}%`,
                        width: `${toPct(cell.end) - toPct(cell.start)}%`,
                        minWidth: 2,
      ...
      {hover && (
        <CursorTooltip x={hover.x} y={hover.y}>
          <div className="mb-1 text-muted-foreground">
            {new Date(hover.cell.ts).toLocaleString()}
          </div>
          <div className="grid grid-cols-[auto_1fr_auto] items-center gap-x-2 gap-y-0.5">
            <span
              className="inline-block size-2.5 rounded-full"
              style={{ backgroundColor: colorByState[hover.cell.state] }}
            />
            <span className="text-muted-foreground">{hover.lane}</span>
            <span className="text-right font-medium">{hover.cell.state}</span>
          </div>
        </CursorTooltip>
      )}
    </div>
