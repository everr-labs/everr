...
describe("bucketSeconds", () => {
  ...
  it("targets the bucket count and scales with the range", () => {
    const oneHour = bucketSeconds(
      at("2026-06-10T09:00:00Z"),
      at("2026-06-10T10:00:00Z"),
      500,
    );
    const sevenDays = bucketSeconds(
  ...
  it("honours the target bucket count", () => {
    ...
    expect(
      bucketSeconds(at("2026-06-10T09:00:00Z"), at("2026-06-10T10:00:00Z"), 10),
    ).toBe(600);
  });
  ...
  it("picks the smallest interval >= ideal (histogram case)", () => {
    // 1h / 60 = 60s exactly.
    expect(
      bucketSeconds(at("2026-03-09T00:00:00Z"), at("2026-03-09T01:00:00Z"), 60),
    ).toBe(60);
  });
  ...
  it("extends past one day for a long range instead of capping", () => {
    ...
    expect(
      bucketSeconds(at("2026-01-01T00:00:00Z"), at("2026-12-31T00:00:00Z"), 80),
    ).toBe(5 * 86400);
  });
