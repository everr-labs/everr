...
// Three ordered cases: an explicit endpoint override wins (trusted to need
// no header), a public origin-bound key ships to the hosted ingest with a
// Bearer header, dev falls back to the local collector. A keyless production
// build resolves to `null` so the SDK never builds an emitter at all.
...
export function resolveTransport(options: {
  ...
  if (endpoint) return { logsUrl: `${endpoint}/v1/logs` };
  if (key) {
    return {
