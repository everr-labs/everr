...
`everr telemetry` is our first answer to that. It starts a local collector, receives OTLP from local services, and exposes `logs` and `traces` commands so you can inspect what's happening on your machine from the CLI. Metrics aren't wired up yet — that's next.

That's useful on its own, but the bigger reason we built it is what it opens up for Everr. It gives coding assistants a way to get runtime context about the local stack directly, instead of making the developer manually copy logs or trace snippets into the prompt every time. That copy-paste step is exactly the kind of useless-human-in-the-loop work we want to cut — the assistant should fetch what it needs on its own. `everr telemetry ai-instructions` is an early version of that idea: a path for an assistant to discover how to inspect local behavior without custom hand-holding every time.
...
**`everr watch` redesign.** We rewrote `everr watch` as an event-driven, append-only stream. That makes it easier for humans to follow and much easier for coding assistants to reason about, because they don't need to re-parse a constantly repainting terminal line. ANSI escape codes in imported logs are also stripped by default, which makes pasting a failure into a chat or an agent's context actually readable.

**Log search improvements.** The `logs` command now supports `--egrep`, validated against the same regex engine ClickHouse uses, plus `--log-failed` and `--job-id` for the common "show me the failing logs" workflows.
...
Next up, we want to keep pushing on both fronts. On the CI side, that means richer desktop run monitoring, better notification content, and the Vercel log drain integration we mentioned last time. On the local side, it means making `everr telemetry` and related commands even more useful for assistant-driven debugging, with outputs and workflows that are easier for agents to consume directly.
