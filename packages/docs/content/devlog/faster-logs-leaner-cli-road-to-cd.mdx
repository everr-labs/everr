...
description: "We shipped a shared UI package, virtual scrolling for logs, a redesigned CLI, streaming onboarding, and a lot of desktop app polish."
date: "2026-04-01"
author: Everr Team
commits: 123
prs: 28
...
Our docs site, desktop app, and web app had each grown their own copies of the same components: buttons, panels, utility functions, Tailwind configs. Changing a color token meant updating three codebases. We'd inevitably miss one.

We extracted everything into a shared `@everr/ui` package and consolidated dependencies through a pnpm catalog. No user-facing change, but every new component we build now exists in one place. For a small team shipping across three surfaces, that's a real multiplier.
...
Importing a repository during onboarding previously showed a spinner with no progress indication. For an import that can take minutes depending on repo size, that's a poor first experience.

The frontend now shows a real-time progress bar that fills proportionally as workflows and runs are imported. Onboarding is someone's first interaction with the product. It needs to feel responsive.
...
**Vitest support.** The test collector previously required Vitest's JSON reporter. It now parses the default reporter output as well, removing a configuration step for teams onboarding.
...
Restyling the desktop app notifications to match the web app's look and feel, cost analysis, search within virtualized logs, and tighter AI agent integration. We want to make it easier for agents to query pipeline state, understand failures, and validate the code they write before it reaches production.

We're also starting to look beyond CI into real CD observability. First up: exploring integration with Vercel via their log drains, so deployment pipelines get the same visibility that build pipelines already have in Everr.
