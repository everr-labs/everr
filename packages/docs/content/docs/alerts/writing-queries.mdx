...
- **Every alert must be actionable.** If no one would do anything when it fires, it shouldn't fire. Link a notebook runbook that gives responders the visuals, context, and next steps they need to debug the problem.
...
Set `spec.notebook` to the notebook slug. A bare slug resolves in the alert's own project; use `project/slug` to link across projects.
...
metadata:
  ...
  project: platform
spec:
  notebook: api-error-rate-runbook   # platform/api-error-rate-runbook
...
`everr apply` fails if the linked notebook does not exist in the same apply batch or already in Everr.
