...
If the effective repoid changes (for example after deleting a legacy `everr.yaml` with a UUID repoid or after a GitHub org/repo rename), run the switch as a transfer:

```sh
everr apply ./everr --transfer-from "<old-repoid>"
```

In one transaction this moves everything the old repoid owns to the new identity, then reconciles the tree as usual: unchanged resources carry over untouched (alerts keep their runtime state and history), changed ones update, and anything no longer in the tree is pruned. Use `--dry-run --transfer-from` first to preview the full effect. Without a transfer, an apply under the new identity fails with a cross-repo ownership conflict that names the old repoid; `--adopt` remains the targeted alternative for deliberately taking over individual resources from another repo.
