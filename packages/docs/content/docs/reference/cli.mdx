...
- `everr apply <dir> --source=<id>` — reconcile a directory of dashboard definitions (`.yaml`/`.yml`/`.json`) into Everr. Makes the source's dashboards exactly match the tree: new files are created, changed files updated, and removed files **deleted**. See [Dashboards](/docs/dashboards).
  ...
  Authentication: uses `EVERR_API_TOKEN` when set (for CI), otherwise your local `everr cloud login` session.
  - `EVERR_API_TOKEN` an organization ingest key, used as a bearer token
  - `EVERR_API_URL` API base URL when using a token (falls back to the URL persisted by a prior login)
