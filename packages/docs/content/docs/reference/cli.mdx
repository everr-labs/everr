...
- `everr apply <dir>` — reconcile a directory of resource definitions (`.yaml`/`.yml`/`.json`) into Everr. Apply discovers `kind: Dashboard` and `kind: AlertRule` documents under the directory. The directory must contain an `everr.yaml` (or `everr.yml`) at its root declaring the projects it manages (`projects: [default, platform]`); apply errors without it. That list is the authoritative reconcile scope — apply reads and prunes only the declared projects, and there is no implicit `default`. Within those projects it makes the live resources match the tree: new files are created, changed files updated, and removed files **deleted**. Every dashboard's `metadata.project` (optional, defaults to `default`) must appear in the list or apply rejects it. See [Dashboards](/docs/dashboards) and [Alerts](/docs/alerts).
  - `<dir>` directory to read resource files from (recursively); must contain an `everr.yaml` manifest at its root; the directory structure becomes folder paths
