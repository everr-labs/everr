...
- `everr apply <dir>` — reconcile a directory of dashboard definitions (`.yaml`/`.yml`/`.json`) into Everr. Makes each project's dashboards exactly match the tree: new files are created, changed files updated, and removed files **deleted**. A dashboard's project comes from its `metadata.project` (optional, defaults to `default`); pruning is scoped per project, so only projects present in the applied directory are affected. See [Dashboards](/docs/dashboards).
