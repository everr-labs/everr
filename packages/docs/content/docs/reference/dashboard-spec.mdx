...
| Parameter | Value |
| --- | --- |
| `{from:String}` / `{to:String}` | The dashboard's selected time range, as UTC datetimes in `YYYY-MM-DD HH:MM:SS` form. The start rounds down and the end rounds up to a whole second, so a `DateTime` column such as `TimeUnix` and a `DateTime64` column such as `Timestamp` both accept a direct comparison. |
