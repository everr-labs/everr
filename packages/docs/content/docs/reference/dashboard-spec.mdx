...
| Parameter | Value |
| --- | --- |
| `{from:String}` / `{to:String}` | The dashboard's selected time range, as UTC datetimes in `YYYY-MM-DD HH:MM:SS.mmm` form (millisecond precision; compare them directly rather than assuming whole seconds). |
