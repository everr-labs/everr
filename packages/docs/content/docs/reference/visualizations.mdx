...
Rows that land on the same (time, bucket) cell are summed. All-numeric buckets sort largest-at-top like a y-axis; categorical buckets keep first-seen order.
...
Nodes are the distinct endpoints, sized by the total weight through them; duplicate (source, target) rows sum, and self-loops are dropped.
