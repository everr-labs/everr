...
Horizontal lanes of discrete colored cells, one per sample: periodic health checks, cron-job runs, CI builds per interval, SLO probes. Supports drag-to-zoom (writes the new range to the URL). Data shape: see [Status history](#status-history).
