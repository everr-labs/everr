...
| `yAxis.min` | number or `auto` | `auto` | Exact lower bound. Values below it are clipped. `auto` fits the data. |
| `yAxis.max` | number or `auto` | `auto` | Exact upper bound. Values above it are clipped. `auto` fits the data. |

Omitting `yAxis` or either bound has the same effect as setting that bound to `auto`. With both bounds automatic, the axis fits the data with padding and readable ticks instead of forcing zero. Set only `min: 0` for a zero baseline on positive data. When both bounds are numeric, `min` must be less than `max`. Stacked charts include zero in their automatic range.
