...
Set `spec.runbook` on the alert, and ship a runbook whose panels show exactly what the alert measured:
...
spec:
  ...
  query: |
    # same error-rate query as the alert-on-error-spikes guide:
...
spec:
  ...
  plugin: { kind: TimeSeriesChart, spec: { unit: "%" } }
  queries:
    - kind: ClickHouseSQL
      spec:
        plugin:
...
2. Check whether the rate is still climbing on the panel above.
...
Never paste a raw ```sql``` block for a responder to run by hand. Panels render live against the time picker; SQL blocks in prose are dead text.

Multi-page runbooks and shared panels: [Track infra health](/docs/guides/track-infra-health) · full format: [Runbook spec](/docs/reference/runbook-spec).
