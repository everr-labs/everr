...
spec:
  ...
  query: |
    ...
    SELECT ServiceName, ... AS value FROM logs ...
```
...
`everr apply` fails if the linked runbook doesn't exist in the same batch or in Everr, so the link can't go stale.

## If the runbook is in another project

Use `project/slug`; a bare slug resolves in the alert's own project.
