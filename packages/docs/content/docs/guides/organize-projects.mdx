---
title: Organize resources into projects
description: Folders come from directories, isolation comes from metadata.project, and multi-repo ownership comes from the repoid in everr.yaml.
---

Folders come from directories; isolation comes from `metadata.project`; multi-repo ownership comes from the `repoid` in `everr.yaml`.
...
The `everr/` directory must contain an `everr.yaml` (or `.yml`) at its root declaring a stable `repoid` — the apply ownership boundary: `everr apply` reconciles exactly what was previously applied under this id, nothing else. Use one stable id per repo (a UUID is a good default); never reuse one across unrelated repos, and never change it on an existing setup (that orphans everything applied under the old id — the old resources stay live but unmanaged). It's the only key the manifest accepts. Give each repo its own `repoid` and applies never collide — `everr apply` can never touch another repo's (or org's) resources:
...
Apply each repo independently with `everr apply ./everr` — once in the platform repo, once in the payments repo, entirely separately. Within a repoid, `everr apply` reconciles live state to match the files exactly — creates new resources, updates changed ones, deletes removed ones, no separate prune step; moving a file relocates its resource, emptying the tree removes its last resource. This is safe because reconciliation only ever touches the declared repoid — see [Observability as code](/docs/concepts/observability-as-code) for the full apply model. Always stage into a preview first:
...
By default `everr apply` uses your `everr cloud login` session and prints the destination org before writing. CI runners don't include the CLI by default — install it with `everr-action` before calling `everr apply`. For CI and other non-interactive contexts, set `EVERR_API_KEY` to an org API key with the `apply` scope (the same `ek_` keys you mint for ingest — the Capabilities column shows what each key can do). Legacy `EVERR_API_TOKEN` is still accepted. If `EVERR_API_URL` isn't set, the base URL falls back to the one saved by a prior `everr cloud login`.
...
`--yes` (or `-y`) skips the confirmation prompt apply shows before writing — required in non-interactive contexts like CI or piped scripts; use `--preview` instead to stage changes without writing (previews never prompt). Gate the apply on your default branch so resources change only after review. To preview in a pull request, apply with an explicit preview name (PR checkouts are detached, so the branch can't be inferred) and post the printed link on the PR:

```sh
everr apply ./everr --preview "pr-${{ github.event.number }}"
```

The preview expires on its own 7 days after its last apply; merging triggers the main-branch workflow above, shipping the changes live. See the [`everr apply` CLI reference](/docs/reference/cli#dashboards-gitops) for the full flag and environment list.
