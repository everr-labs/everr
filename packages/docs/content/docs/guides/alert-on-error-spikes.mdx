---
title: Alert on error spikes
description: An AlertRule that fires per service when the error rate crosses a threshold with enough volume to matter.
---

This rule fires one instance per service whose error rate exceeds 5% over 15 minutes — and stays quiet on tiny samples:
...
Every row is a firing instance; empty result = resolved ([how alerts evaluate](/docs/concepts/how-alerts-evaluate)).
