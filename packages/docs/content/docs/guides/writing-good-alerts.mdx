...
The query keeps both healthy and breaching services available for visualization. The condition turns only rows with a guarded error rate above 5% into firing instances ([how alerts work](/docs/concepts/how-alerts-work)).

## Choosing the threshold and window

- Guard percentage alerts with a minimum-volume signal. One failure out of one request is a 100% error rate that should notify no one; `if(total >= 100, error_rate, 0) AS value` keeps tiny samples below the configured threshold.
