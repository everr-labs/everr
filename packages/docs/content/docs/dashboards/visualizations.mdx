...
Plots geographic data on a world map, either as latitude/longitude markers (`points`) or by shading countries (`choropleth`). In points mode it renders **all** query frames, giving each query its own marker color; in choropleth mode it merges every frame by **summing** the value of each region. Region codes that don't resolve to a known country and points with invalid coordinates are dropped.
...
| `showLegend` | boolean | `true` | Per-query swatches (points) or the color ramp (choropleth). |
| `colorScheme` | enum | `blue` | One of `blue`, `green`, `orange`, `red` — marker base / choropleth ramp. |
| `min` | number | — | Lower bound of the color/size domain. Auto-derived from data when unset. |
