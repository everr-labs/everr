...
A panel's `plugin.kind` selects its visualization and `plugin.spec` configures it. Everr ships six visualizations. An unrecognized `kind` renders an "Unknown visualization" placeholder rather than breaking the dashboard.
