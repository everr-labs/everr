...
A panel's `plugin.kind` selects its visualization and `plugin.spec` configures it. Everr ships nine visualizations. An unrecognized `kind` renders an "Unknown visualization" placeholder rather than breaking the dashboard.
