...
- **`allowAllValue`** — add an "All" entry. When selected, `$name` expands to every loaded option. Set `customAllValue` to substitute a fixed string instead (for example `customAllValue: "%"` for a `LIKE`).
