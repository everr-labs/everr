...
- `everr watch` (waits until local `HEAD` commit appears in runs; use `--commit <sha>` to target a specific commit, `--attempt <n>` to target a specific retry attempt; exits non-zero if any completed run finishes with `failure`)
