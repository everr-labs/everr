...
Generate your own UUID (for example with `uuidgen`) and keep it stable: changing it makes apply treat the rules as a different repo, so it deletes the old ones and recreates them. The manifest accepts only `repoid`; any other key is rejected. See the [manifest reference](/docs/reference/alert-spec).
