...
```text
Use the everr-setup-telemetry skill. First look at what telemetry this repo
actually emits in Everr, then propose the single alert most worth having (a
user-facing symptom, not a cause), ground its threshold in the real baselines,
and write the rule. Tell me why that signal is the one worth paging on.
```
