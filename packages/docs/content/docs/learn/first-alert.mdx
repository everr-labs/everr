...
Apply scopes everything to your repository's identity (its `repoid`), inferred from the repo's `origin` remote as `host/owner/repo`. No manifest file is needed; an optional `everr.yaml` can pin an explicit `repoid` instead. See the [manifest reference](/docs/alerts/alert-spec).
