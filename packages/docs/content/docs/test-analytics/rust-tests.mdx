...
Run your Rust tests on a nightly toolchain with `--report-time` so libtest emits per-test durations:

```bash
cargo test -- --report-time -Z unstable-options
```
...
```yaml
- name: Run tests
  run: cargo test -- --report-time -Z unstable-options
```
