 ...
 * SDK makes one `GET <url>` span for each static resource, for example a
 * script, a CSS file, an image, or a font. It also makes one
 * `long_animation_frame` span for each interval when the main thread stops in
 * the same window. An SPA navigation never opens this window again.
 *
