 ...
 * SDK makes one `PageLoad` root span from the time origin to the LCP, and one
 * `pageLoad.asset.<initiator_type>` child span for each static resource, for
 * example a script, a CSS file, an image, or a font. It also makes one
 * `long_animation_frame` child span for each interval when the main thread
 * stops in the same window. An SPA navigation never opens this window again.
 *
