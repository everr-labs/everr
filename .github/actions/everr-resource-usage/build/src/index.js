...
function resolveActionRoot(entrypointPath = fileURLToPath(import.meta.url)) {
    return path.resolve(path.dirname(entrypointPath), "..");
}
function buildRuntimePaths(env = process.env) {
    const runnerTemp = env.RUNNER_TEMP || os.tmpdir();
    const baseDir = path.join(runnerTemp, "everr-resource-usage", `${env.GITHUB_RUN_ID || "0"}-${env.GITHUB_RUN_ATTEMPT || "0"}-${env.GITHUB_JOB || "job"}`);
    return {
        baseDir,
...
async function startResourceUsage({ env = process.env, fsModule = fs, fspModule = fsp, saveState = core.saveState, getInput = core.getInput, info = core.info, warning = core.warning, now = () => new Date(), spawnImpl = spawn, fetchImpl = fetch, } = {}) {
    ...
    const checkRunId = await discoverCheckRunId({
        ...
        readdir: fspModule.readdir.bind(fspModule),
        fetchImpl,
        now,
    });
    if (!checkRunId) {
...
async function finalizeAndUploadResourceUsage({ env = process.env, fspModule = fsp, readState = core.getState, info = core.info, warning = core.warning, now = () => new Date(), execFileImpl = execFileAsync, resolveFilesystemInfo = resolveWorkspaceFilesystemInfo, uploadArtifactImpl = (name, files, rootDirectory, options) => artifactClient.uploadArtifact(name, files, rootDirectory, options), } = {}) {
...
async function ensureSamplesFile(samplesPath, fspModule = fsp) {
    if (!samplesPath) {
        return;
    }
...
async function stopSampler(pidPath, warning = core.warning, fspModule = fsp) {
    if (!pidPath) {
        return;
    }
    let rawPid;
...
async function discoverCheckRunId({ env = process.env, getInput = core.getInput, info = core.info, warning = core.warning, readFile = (filePath) => fsp.readFile(filePath, "utf8"), readdir = (directoryPath, options) => fsp.readdir(directoryPath, options), fetchImpl = fetch, now = () => new Date(), } = {}) {
    ...
    try {
        ...
        if (!checkRunId) {
            warning("resource-usage discovery skipped: could not match the current workflow job to a check run");
            return null;
        }
...
async function resolveJobNameHints({ env = process.env, readFile = (filePath) => fsp.readFile(filePath, "utf8"), readdir = (directoryPath, options) => fsp.readdir(directoryPath, options), } = {}) {
    const hints = new Set();
    if (env.GITHUB_JOB) {
...
async function resolveWorkflowJobName({ env = process.env, readFile = (filePath) => fsp.readFile(filePath, "utf8"), readdir = (directoryPath, options) => fsp.readdir(directoryPath, options), } = {}) {
...
async function resolveWorkflowPath({ env = process.env, readFile = (filePath) => fsp.readFile(filePath, "utf8"), readdir = (directoryPath, options) => fsp.readdir(directoryPath, options), } = {}) {
    const workspacePath = env.GITHUB_WORKSPACE || process.cwd();
...
async function listWorkflowRunJobs({ env = process.env, token, fetchImpl = fetch, }) {
...
function selectCheckRunId({ jobs, hints = [], runnerName = "", now = () => new Date(), }) {
    ...
    if (normalizedHints.length > 0) {
        const hintMatches = candidates.filter((job) => typeof job.name === "string" && normalizedHints.includes(job.name));
    ...
    const startedCandidates = candidates
        ...
        .sort((left, right) => Math.abs(left.startedAt - referenceTime) -
...
export { artifactNameForCheckRun, buildRuntimePaths, discoverCheckRunId, ensureSamplesFile, finalizeAndUploadResourceUsage, formatError, isActiveJob, listWorkflowRunJobs, parseCheckRunId, resolveActionRoot, resolveJobNameHints, resolveWorkspaceFilesystemInfo, resolveWorkflowJobName, resolveWorkflowPath, selectCheckRunId, startResourceUsage, stopSampler, workflowPathFromRef, };
