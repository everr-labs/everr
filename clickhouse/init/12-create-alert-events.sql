...
-- Narrow correlation projection: an engineer reading application output
-- around an incident sees the alert transition inline. Fired and resolved
-- only, live write source only (a re-driven stream cannot feed an append-only
-- projection), identity but no evidence, samples, context or delivery
-- targets: those live in the typed table, and a second full copy could
-- disagree with it.
...
SELECT
  ...
  -- The service the alert concerns, resolved at write time, so a filter on
  -- that service returns its alert transitions.
  ...
  map(
    ...
    -- Marks alert rows without overloading service.namespace, which users'
    -- own services may already set.
    'everr.signal', 'alert',
