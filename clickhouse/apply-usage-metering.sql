-- Apply usage metering to an existing ClickHouse cluster. This file is
-- intentionally repeatable and contains no historical backfill.
--
-- Follow clickhouse/USAGE_METERING_ROLLOUT.md. The primary path runs the
-- effective collector-user preflight, applies this file while recording the
-- UTC cutover window, and performs schema and per-signal validation:
--   ./clickhouse/usage-metering-rollout.sh preflight <collector client args>
--   ./clickhouse/usage-metering-rollout.sh apply <admin client args>
--
-- Raw apply escape hatch, only after the same preflight has passed:
--   clickhouse-client --user default --password '<ADMIN_PASSWORD>' --multiquery \
--     < clickhouse/apply-usage-metering.sql
