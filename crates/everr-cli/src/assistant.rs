...
pub fn init_assistants(assistants: &[AssistantKind]) -> Result<()> {
    let assistants = assistants
        ...
        .collect::<Vec<_>>();
    core_assistant::init_assistants(&assistants, build::command_name())?;
...
pub fn refresh_existing_managed_prompts() -> Result<Vec<AssistantKind>> {
    core_assistant::refresh_existing_managed_prompts(build::command_name()).map(|assistants| {
        assistants
            .into_iter()
...
mod tests {
    ...
    use super::{from_core_assistant, to_core_assistant};
