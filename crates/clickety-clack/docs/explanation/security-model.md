...
Email recipient addresses and Telegram chat ids are structural configuration and
are stored as-is. Webhook URLs and Telegram bot tokens are encrypted at rest and
masked as `"***"` on API responses.
...
This closes DNS rebinding and redirect-to-internal paths; deployment-level egress
policy remains a second boundary. `CC_ALLOW_PRIVATE_WEBHOOKS=1` is the dev-only
