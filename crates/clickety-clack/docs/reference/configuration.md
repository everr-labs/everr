...
| `CC_ALLOW_PRIVATE_WEBHOOKS` | `0` | Set to `1` (or `true`) to allow generic webhook, Slack, and Discord channel URLs targeting private, loopback, or link-local addresses and `localhost`. Dev/compose escape hatch only; never set it in a multi-tenant deployment. Structural rules (http/https scheme, no userinfo) and the no-redirect policy always apply. The `api` and `dispatcher` roles read it. |
