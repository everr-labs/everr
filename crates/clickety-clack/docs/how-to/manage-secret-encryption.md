...
- **What's *not* encrypted:** webhook channel URLs and email recipient addresses
  are stored structurally (not treated as secrets); only Slack URLs, PagerDuty
  routing keys, Telegram bot tokens, and subscription webhook URLs are
  encrypted. Receivers hold channel names only and carry no secrets at all.
