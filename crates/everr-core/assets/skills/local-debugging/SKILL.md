...
8. If data is empty or stale, treat setup as the bug: verify the app is running, exporters point at `everr local endpoint`, and the collector is up. Invoke the `local-telemetry-setup` skill to fix collector, exporter, or `service.name` configuration.
