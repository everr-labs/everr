...
- Browser `fetch` requests, slow interactions, and page-load capture are **CLIENT spans** in `traces`. Custom segments from the exported `tracer` default to **INTERNAL spans**.
