...
| `pageLoad` | Root of the first-load trace, from `pageLoad()`; starts at the document time origin and ends at LCP (the last `largest-contentful-paint` entry; the `load` event end in a browser without LCP; the hidden event or the window ceiling before both). Every SDK span that starts while it is open is its child (the `pageLoad.*` spans, fetch spans and their server spans, `slow_interaction`), so one `TraceId` groups the whole load. Rule is by time, not cause: a fetch from an unrelated timer during the load joins too. Absent when `pageLoad()` is not composed, or in a session `sampled()` left out | `everr.browser.page_load.end`: `lcp`, `load`, `hidden`, or `ceiling` (what ended the root; filter on `lcp` when the duration must mean time to LCP) |
