...
- Browser `fetch` and XHR requests, slow interactions, and page-load capture are **CLIENT spans** in `traces`. Custom segments from the exported `tracer` default to **INTERNAL spans**.
...
| --- | --- | --- |
| Application-defined name (e.g. `prepare checkout`) | Manual segment from `tracer.startSpan()` or `tracer.startActiveSpan()`. The latter stays active until `end()` in the browser; custom and built-in spans started during that time join the most recently active span's trace. Overlapping interactions share this stack, not isolated async contexts | Context envelope plus caller-supplied attributes. Prefer bounded names and `everr.*` for custom attributes. `recordException()` adds `exception.type` and `exception.message`; `setStatus({ code: 2 })` marks failure |
| `<METHOD> <route>` (e.g. `GET /api/posts/{id}`) | One per browser `fetch` or XHR; the server spans parent to it, so `TraceId` joins frontend to backend. During the first load (while the `pageLoad` root is open) it is a child of that root and shares its `TraceId`; after that it is its own trace root | HTTP client semconv: `http.request.method`, `http.response.status_code`, `url.full` (query-stripped request URL), `url.template`, `server.address`; 4xx/5xx set `StatusCode = 'Error'` with `error.type` |
...
XHR spans start at `send()` and end on completion, timeout, network failure, or cancellation. Network failures set `error.type = NetworkError`; timeouts set `error.type = TimeoutError`. Explicit aborts and requests replaced by `open()` end without an error status. A missing HTTP response has no `http.response.status_code`. Shutdown detaches listeners and discards unfinished XHR observations without aborting application requests. Everr owns XHR trace propagation. Caller-supplied `traceparent` headers are not detected; setting one alongside `network()` causes XHR to append the SDK value, producing an invalid trace header.
