...
| `PageLoad` | Root of the first-load trace, from `pageLoad()`; starts at the document time origin and ends at LCP (the last `largest-contentful-paint` entry; the `load` event end in a browser without LCP). The asset and `long_animation_frame` spans below are its children, so `TraceId` groups one load | Envelope only (`url.full`, route, session) |
...
| `long_animation_frame` | Main-thread stall during the first load, a child of `PageLoad` | `everr.browser.long_animation_frame.*`: `blocking_duration`, `script_duration`, `style_and_layout_duration`, `unattributed_duration`, and the longest script as `script.source_url`, `script.function_name`, `script.invoker_type`, `script.duration` |
