...
3. **Visualizations are nine kinds with simple specs:** `TimeSeriesChart`, `Table`, `StatChart`, `GaugeChart`, `GeoMap`, `Treemap`, `StateTimeline`, `StatusHistory`, `Heatmap`. They infer structure from the columns you `SELECT` — there is no `yAxis`, `legend`, `columnSettings`, `format.unit`, etc.
...
panels:
  error-rate:                            # panel key — referenced by the layout's $ref
    ...
    spec:
      ...
      plugin:
        kind: TimeSeriesChart            # TimeSeriesChart | Table | StatChart | GaugeChart | GeoMap | Treemap | StateTimeline | StatusHistory | Heatmap
