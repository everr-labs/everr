...
description: Use when creating, editing, or applying Everr as-code resources (dashboards, runbooks, alert rules), Perses-format dashboard YAML, panels, ClickHouse queries, ```panel blocks, AlertRule YAML, the everr.yaml manifest, the `everr apply` CLI, or branch previews (`everr apply --preview`).
---
