...
From the Errors UI a User picks an Error and gets an agent-agnostic handoff prompt; the Agent pulls full context through a new `errors` CLI surface, fixes the code, and writes its findings back as an Investigation and a Resolution. Everything written about an Error (Investigations, Resolutions, status changes) is itself telemetry: append-only log events keyed by the Error's Fingerprint, stored in the existing logs tables on both surfaces (Cloud and Collector), and therefore queryable by any Agent through the same SQL surfaces as every other Signal (see ADR 0004).
...
21. As an Agent, I want Investigations and Resolutions readable through the plain SQL query surfaces, so that I can consume them like any other Signal without a bespoke API.
...
25. As a User, I want status changes and Investigations timestamped and append-only, so that history is never silently rewritten.
...
- **No new schema anywhere** (ADR 0004). Triage events are plain log records in the existing logs tables on both surfaces, keyed by tenant and Fingerprint, with a dedicated attribute namespace (`everr.error.*`), the markdown body in the log Body, and an event-type attribute distinguishing investigation, resolved, ignored, reopened.
...
- **Write path is a generic log-event emitter, not error-specific**: one interface with two backends, a REST insert endpoint on the Cloud (authenticated, tenant stamped server-side) and OTLP ingestion into the Collector locally. Error triage is its first consumer.
- **CLI surface** mirrors the existing local/cloud split: `errors list | show | investigate | resolve | ignore | reopen` under both the local and cloud command families. `show` returns full Error context including the event timeline. Markdown bodies accepted via stdin or file flag.
...
- **Errors repository seam** (existing fake-execute-client pattern): status derivation, status filter, event timeline query, and Regression-rule SQL shape are tested exactly like the current repository tests.
...
- **Event-write seam** (existing data-layer test pattern used by alerts): the insert path validates input, stamps tenant and author server-side, and emits the correct log-row shape, with the ClickHouse client mocked.
...
- A dedicated MCP tool for errors (the MCP `query` tool already reads the same events; a typed tool can come later).
- Dedicated ClickHouse or Postgres tables for triage state, and any fingerprint re-linking or migration tooling.
- Editing or deleting past events; corrections are new appended events.
...
- ADR 0004 ("Error triage state is append-only telemetry, not a database row") records the storage decision, the rejected alternatives, and the accepted consequences (no edits, TTL expiry with log retention, orphaning on fingerprint-algorithm change, heavier list query, tenant policy as a correctness requirement).
- The domain glossary defines all terms used here (Error, Occurrence, Fingerprint, Investigation, Resolution, Status, Regression).
- Triage state expiring with log retention is acceptable by design: an Error with no Occurrences inside the retention window is not listed anyway.
- The generic log-event emitter should be designed so future features (annotations on traces, deploy markers) can reuse it without change.
