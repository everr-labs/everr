...
Errors gain a derived Status (open, resolved, ignored) with a version-aware Regression rule: a resolved Error reopens only when an Occurrence arrives from a service version outside the set known at resolve time, so same-version stragglers from old deploys do not flap the status.
...
- **Regression rule**: an Occurrence reopens a resolved Error iff its `service.version` is outside the versions the Resolution snapshotted at resolve time. An Occurrence with no `service.version`, or a Resolution with no version knowledge (empty snapshot), degrades to a plain timestamp comparison against the Resolution. Version membership only, never semver or SHA comparison. Ignored is sticky: only a manual status change lifts it.
- The Resolution event carries `resolved_versions`: the versions currently in telemetry for the services the Error occurred in, snapshotted by the write module when the Resolution is recorded. Paying the version scan once per Resolution (rare) instead of on every list read keeps the summary derivation to two bounded scans: one of the small events table for triage state, delivered as query parameters into the single per-page logs scan. (Supersedes the earlier first-seen-time rule, which needed an unbounded per-read scan of log history.)
...
- **Real-ClickHouse integration seam** (new, env-gated and skipped when the env var is absent, mirroring the existing real-Postgres integration test pattern): proves the Regression rule end-to-end with seeded Occurrences and Resolutions carrying version snapshots, including the versionless fallback, the empty-snapshot fallback, same-version stragglers, and sticky ignore.
