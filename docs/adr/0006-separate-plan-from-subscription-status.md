...
The pending Organization name, generated slug, Owner identifier, purpose, and schema version travel as Polar Customer and checkout metadata. Both the authenticated success callback and the signed subscription webhook validate that metadata and invoke the same finalizer. The finalizer uses the generated unique Organization slug as its idempotency key, so concurrent or retried delivery reuses the Organization and subscription link. A repeated creation attempt by the same Owner resumes an open checkout, continues finalization for a confirmed checkout, or reuses the provisional Customer after a terminal checkout. A different Owner cannot claim that Customer. No local checkout-intent record is persisted.
