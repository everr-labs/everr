...
With the target shape fixed by [Decide SDK packaging, ingestion path, and dual privacy modes](013-sdk-shape-ingestion.md) (side-effect `telemetry.ts` module, cookieless init, PostHogProvider removed): how does the homepage get from PostHog to `@everr/otel-web`? Decide whether there is a parallel-run window with both SDKs emitting (and for how long), what verification proves the Everr side is trustworthy before PostHog is removed (which metrics are compared, what counts as parity given cookieless sessions replace unique visitors), and the removal steps (posthog-js dependency, env vars, PostHog project teardown or retention).

## Resolution

**Parallel-run, two PRs.** PR 1 ships `@everr/otel-web` on the homepage alongside PostHog; both emit simultaneously for roughly two weeks. PR 2 removes PostHog once verification passes. Rationale: verification is only meaningful on overlapping traffic, the SDKs do not interact, and rollback during the window is deleting one file.
