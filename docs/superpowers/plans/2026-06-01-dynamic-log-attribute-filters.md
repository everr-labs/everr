...
> **Type-debt cleanup (added during execution):** Because `attributes` became a *required* member of the inferred query-input types in Task 1, the existing hand-constructed input literals in `explorer.test.ts`, `totals.test.ts`, `histogram.test.ts`, `repository.test.ts`, and `options.test.ts` no longer satisfy `tsc` (they run fine under vitest, which strips types, but `pnpm typecheck` flags them). This task is where the package returns to full typecheck-green. In each of those test files, update every query-input literal: **remove** the `repos: []` field and **add** `attributes: []` (for the explorer/totals/histogram/repository inputs). The `options.test.ts` fake repo object also needs `attributeKeys`/`attributeValues` added (it already constructs a partial `LogsRepositoryLike` mock — extend it, or cast as needed to match the existing mock style). After this task, `pnpm --filter @everr/telemetry-explorer typecheck` must be fully clean.

> **UI-commit note (added during execution):** The UI component files in Tasks 10–12 (`attribute-key-picker.tsx`, `attribute-filter-row.tsx`, `attribute-filter-section.tsx`) have no co-located tests, so `fallow` (the dead-code pre-commit hook, which treats test files as entry points) flags them as unused until `log-filters.tsx` imports the section in Task 13. `LEFTHOOK_EXCLUDE` is **not honored** by the installed lefthook version, so instead these three components are written into the working tree **without committing** (Tasks 10–12 do their TDD/typecheck verification but skip the commit step). Task 13 wires the section into the already-reachable `log-filters.tsx` and makes a **single normal commit** of all four files together — at which point the whole new UI subtree is reachable and `fallow` passes. Per-component spec/quality review still happens (against the working tree before the combined commit, or against the combined commit diff after).
