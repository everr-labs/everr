...
- **Connection pool (demo)** reports a varying but always healthy pool utilization. Use it to watch a non-firing instance. Its `demo-broken-query` slug is retained so existing event history stays attached.
